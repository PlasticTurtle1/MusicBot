	...
	*/
	async createPlaylist(bot, channel, user, playlistName, searchQuery) {
		...
		switch (res.loadType) {
			...
			case 'TRACK_LOADED': {
				const tracks = res.tracks.slice(0, user.premium ? 1000 : 800);
	...
	async savePlaylist(bot, channel, user, playlistName, resData) {
		try {
			...
			const embed = new Embed(bot, channel.guild)
				...
				.setFooter({ text: channel.guild.translate('music/p-create:FOOTER', { ID: newPlaylist._id, NUM: newPlaylist.songs.length, PREM: (user.premium) ? '1000' : '800' }) })
				.setTimestamp();
			return embed;
		} catch (err) {
